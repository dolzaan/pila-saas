    ...
    <div className="space-y-4">
      ...
      <details className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <input value={phoneNumber} onChange={(e) => setPhoneNumber(formatPhone(e.target.value))} placeholder="Telefone com DDD" inputMode="tel" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white" />
          <input value={postalCode} onChange={(e) => setPostalCode(formatPostalCode(e.target.value))} placeholder="CEP" inputMode="numeric" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white" />
          <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Rua / avenida" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white sm:col-span-2" />
          <input value={addressNumber} onChange={(e) => setAddressNumber(e.target.value)} placeholder="Número" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white" />
          <input value={province} onChange={(e) => setProvince(e.target.value)} placeholder="Bairro" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white" />
          <input value={complement} onChange={(e) => setComplement(e.target.value)} placeholder="Complemento (opcional)" className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white sm:col-span-2" />
        </div>
      </details>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button type="button" onClick={() => handleSubscribe("PIX")} disabled={isPending || !documentValid} className="app-button">
          Pagar com Pix
        </button>
        <button type="button" onClick={() => handleSubscribe("BOLETO")} disabled={isPending || !documentValid} className="app-button">
          Pagar com boleto
        </button>
        <button type="button" onClick={() => setShowOptions(false)} disabled={isPending} className="text-sm text-gray-400 underline-offset-4 hover:underline">
          Voltar
        </button>
      </div>
      <p className="text-xs text-gray-500">
        Telefone e endereço são exigidos apenas pelo checkout de cartão do Asaas.
      </p>
