...
export default function HowItWorksPage() {
  return (
    <main className={styles.page}>
      <PublicIridescence />
      <header className={styles.header}><div className={styles.headerInner}><Link className={styles.brand} href="/"><Image src="/logo-icon.png" alt="" width={40} height={40} priority /><span>Pila</span></Link><nav className={styles.nav}><Link href="/features">Recursos</Link><Link href="/how-it-works">Como funciona</Link><Link href="/security">Segurança</Link><Link href="/#preco">Preço</Link></nav><div className={styles.headerActions}><Link className={styles.loginLink} href="/login">Entrar</Link><Link className={styles.smallCta} href="/register" data-specular-button>Testar grátis por 7 dias <ArrowRight size={16} /></Link></div></div></header>
      <section className={styles.hero}><div className={styles.heroGrid}><div className={styles.heroCopy}><div className={styles.eyebrow}><Sparkles size={15} /> COMO O PILA FUNCIONA</div><h1>Da conversa ao controle financeiro.</h1><p>Você fala do seu jeito. O Pila interpreta, organiza e transforma cada mensagem em informação útil para o seu dia a dia.</p><div className={styles.heroActions}><Link className={styles.primaryCta} href="/register" data-specular-button>Testar grátis por 7 dias <ArrowRight size={19} /></Link><Link className={styles.secondaryCta} href="/features">Conhecer recursos</Link></div><div className={styles.heroNotes}><span><Check size={15} /> Texto, áudio e foto</span><span><Check size={15} /> WhatsApp e Telegram</span><span><Check size={15} /> Dashboard atualizado</span></div></div></div></section>
      ...
      <section className={styles.finalCta}><div><span>Comece com uma mensagem.</span><h2>Deixe o Pila cuidar da organização.</h2></div><Link className={styles.lightCta} href="/register">Testar grátis por 7 dias <ArrowRight size={19} /></Link></section>
